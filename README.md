...
For infinite medians, we provide the `mad_with_infinities()` function that treats infinities of the same sign as equal values.
...
auto med = quickstats::median<double>(x.size(), x.data(), {});
auto mad = quickstats::mad_with_infinities<double>(x.size(), x.data(), med, {});
```
