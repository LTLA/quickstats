...
TEST(SingleQuantile, Dense) {
...

    {
        ...
        double val = quickstats::SingleQuantileFixedNumber<double>(n, 0)(values.data());
        EXPECT_FLOAT_EQ(val, 0.5);
    }

    {
        auto values = original;
        double val = quickstats::SingleQuantileFixedNumber<double>(n, 0.1)(values.data());
        EXPECT_FLOAT_EQ(val, 0.6);
    }

    {
        auto values = original;
        double val = quickstats::SingleQuantileFixedNumber<double>(n, 0.25)(values.data());
        EXPECT_FLOAT_EQ(val, 0.9);
    }

    {
        auto values = original;
        double val = quickstats::SingleQuantileFixedNumber<double>(n, 0.5)(values.data());
        EXPECT_FLOAT_EQ(val, 1.9);
    }

    {
        auto values = original;
        double val = quickstats::SingleQuantileFixedNumber<double>(n, 0.75)(values.data());
        EXPECT_FLOAT_EQ(val, 6.725);
    }

    {
        auto values = original;
        double val = quickstats::SingleQuantileFixedNumber<double>(n, 0.8)(values.data());
        EXPECT_FLOAT_EQ(val, 8.2);
    }

    {
        auto values = original;
        double val = quickstats::SingleQuantileFixedNumber<double>(n, 1)(values.data());
    ...
    {
        auto values = original;
        auto fval = quickstats::SingleQuantileFixedNumber<float>(n, 1)(values.data());
        EXPECT_FLOAT_EQ(static_cast<double>(fval), 8.8);
    }

    {
        std::vector<float> fvalues(original.begin(), original.end());
        auto val = quickstats::SingleQuantileFixedNumber<double>(n, 1)(fvalues.data());
        EXPECT_FLOAT_EQ(val, 8.8);
    }
}
...
TEST_P(ComplexSingleQuantileTest, Dense) {
    ...
    for (int val = 1; val < 100; ++val) {
        ...
        auto values = original;
        const double Q = quickstats::SingleQuantileFixedNumber<double>(n, prop)(values.data());
    ...
    {
        auto values = original;
        const double Q = quickstats::SingleQuantileFixedNumber<double>(n, 0)(values.data());
        EXPECT_EQ(Q, *std::min_element(original.begin(), original.end()));
    }

    {
        auto values = original;
        const double Q = quickstats::SingleQuantileFixedNumber<double>(n, 1)(values.data());
...
TEST_P(ComplexSingleQuantileTest, Sparse) {
    ...
    for (int val = 0; val < 100; ++val) {
        const double prop = static_cast<double>(val)/100.0;
        quickstats::SingleQuantileFixedNumber<double> Q(n, prop);
...
TEST(SingleQuantile, InvalidArguments) {
    std::string msg;
    try {
        quickstats::SingleQuantileFixedNumber<double>(3, -1);
    ...
    msg.clear();
    try {
        quickstats::SingleQuantileFixedNumber<double>(3, 10);
    } catch (std::exception& e) {
        msg = e.what();
    }
    EXPECT_TRUE(msg.find("[0, 1]") != std::string::npos);
    ...
    msg.clear();
    try {
        quickstats::SingleQuantileFixedNumber<double>(0, 1);
    } catch (std::exception& e) {
        msg = e.what();
    }
...
TEST(SingleQuantile, FloatConversion) {
    ...
    for (int i = 0; i < 100; ++i) {
        std::size_t x = i + static_cast<std::size_t>(1e8);
        ...
        quickstats::SingleQuantileFixedNumber<float> test(x, 1);
...
TEST(SingleQuantile, IntegerInput) {
    quickstats::SingleQuantileFixedNumber<double> test(10, 0.5);
...
TEST(SingleQuantile, Infinities) {
    ...
    quickstats::SingleQuantileFixedNumber<double> test(2, 0.5);
